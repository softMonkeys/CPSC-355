...
equal:   .string "Loop #%d: Exactly 5! (Ya I know this program's stupid too.)\n"
        ...
        .balign 4                   // Instructuions must be word aligned
...
test:   cmp    x19, 10              // Compare loop counter and 10
        ...
        // If loop# < 5
        cmp    x19, 5
        b.gt   elseif               // If loop# > 5, then condition is NOT satisfied, move to elseif (loop# > 5)
        b.eq   else                 // If loop# == 5, then condition is NOT satisfied, move to else (loop# == 5)
...
elseif: adrp   x0, greater
        ...
        // In the conditiona;s we set the arguments for printf(), so let's call it now
...
next:   bl     printf               // Call the printf() function
        ...
        ldp    x29, x30, [sp], 16   // Restore FP and IR from stack, post-increment SP
