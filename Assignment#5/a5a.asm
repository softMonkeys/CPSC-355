...
define(value_r, w24)                                    // value register
define(i_r, w21)                                        // i counter register
define(temp_r, w22)                                     // temporary variable register
// GLOBAL VARIABLES
...
push:       stp     fp, lr, [sp, -16]!                  // Save frame pointer (fp, x29) and link register (lr, x30) to stock, allocating 16 bytes, ore_increment SP
            mov     fp, sp                              // Update frame pointer (fp) to current stack pointer (sp) (after we've increment sp in the last step)
            mov     value_r, w0                         // move the value of "value" into a temporary register 
...
plus:       adrp    base_r, top                         // get the base address of our pointer
            ...
            mov     temp_r, w1                          // move value of top into a temporary register
            add     temp_r, temp_r, 1                   // top++
            str     temp_r, [base_r]                    // store top back to it's memory address
            adrp    base_r, dest                        // get the base address of our pointer
            add     base_r, base_r, :lo12:dest
            str     value_r, [base_r, temp_r, SXTW 2]   // store the value into the stack at index of top
done:       ldp     fp, lr, [sp], 16                    // Restore FP and LR from stack, post-increment SP
            ret                                         // Return to caller

// pop()
            .global pop                                 // Make "pop" visible to the OS
pop:        stp     fp, lr, [sp, -16]!                  // Save frame pointer (fp, x29) and link register (lr, x30) to stock, allocating 16 bytes, ore_increment SP
            mov     fp, sp                              // Update frame pointer (fp) to current stack pointer (sp) (after we've increment sp in the last step)
...
pop1:       adrp    base_r, top                         // get the base address of our pointer
            ...
            adrp    base_r, dest                        // get the base address of our pointer
            add     base_r, base_r, :lo12:dest
            ldr     value_r, [base_r, temp_r, SXTW 2]   // load the value of stack at index of top into value
            sub     temp_r, temp_r, 1                   // top--
            adrp    base_r, top                         // get the base address of our pointer
            add     base_r, base_r, :lo12:top
            str     temp_r, [base_r]                    // store top into memory
            mov     w0, value_r                         // return value
done1:      ldp     fp, lr, [sp], 16                    // Restore FP and LR from stack, post-increment SP
            ret                                         // Return to caller

// stackFull()
            .global stackFull                           // Make "stackFull" visible to the OS
stackFull:  stp     fp, lr, [sp, -16]!                  // Save frame pointer (fp, x29) and link register (lr, x30) to stock, allocating 16 bytes, ore_increment SP
            mov     fp, sp                              // Update frame pointer (fp) to current stack pointer (sp) (after we've increment sp in the last step)
            ...
            add     base_r, base_r, :lo12:top
            ldr     w1, [base_r]                        // load the value of top into w1 register
            mov     temp_r, w1                          // move top into a temporary register
            cmp     temp_r, w19                         // compare if top equals ro stack size minus 1
...
done2:      ldp     fp, lr, [sp], 16                    // Restore FP and LR from stack, post-increment SP
            ret                                         // Return to caller

// stackEmpty()
            .global stackEmpty                          // Make "stackEmpty" visible to the OS
stackEmpty: stp     fp, lr, [sp, -16]!                  // Save frame pointer (fp, x29) and link register (lr, x30) to stock, allocating 16 bytes, ore_increment SP
            mov     fp, sp                              // Update frame pointer (fp) to current stack pointer (sp) (after we've increment sp in the last step)
            adrp    base_r, top                         // get the base address of our pointer
            add     base_r, base_r, :lo12:top
            ldr     w1, [base_r]                        // load the value of top into w1 register
            mov     temp_r, w1                          // move top into a temporary register
            cmp     temp_r, -1                          // compare top with -1
...
done3:      ldp     fp, lr, [sp], 16                    // Restore FP and LR from stack, post-increment SP
            ret                                         // Return to caller

// display()
            .global display                             // Make "display" visible to the OS
display:    stp     fp, lr, [sp, -16]!                  // Save frame pointer (fp, x29) and link register (lr, x30) to stock, allocating 16 bytes, ore_increment SP
            mov     fp, sp                              // Update frame pointer (fp) to current stack pointer (sp) (after we've increment sp in the last step)
            bl      stackEmpty                          // call stackEmpty function
...
done4:      ldp     fp, lr, [sp], 16                    // Restore FP and LR from stack, post-increment SP
            ret                                         // Return to caller
