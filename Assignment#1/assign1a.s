// (unoptimized version) Program to show the minimum of y = 5x^3 + 27x^2 - 27x - 43 in the range -6 <= x <= 6, by stepping through the range one by one in a loop and testing
...
main:   stp    x29, x30, [sp, -16]! // Save frame pointer (fp, x29) and link register (lr, x30) to stock, allocating 16 bytes, ore-increment SP
        ...
        mov    x20, 0               // number (5, 27, -27, -43) register 
        mov    x21, 0               // x register
        mov    x22, 0               // variable (5x^3, 27x^2, -27x) register
        mov    x23, 0               // y register
        mov    x24, 100             // minimum register

        // While loop (pre-test, so check before the fisrt iteration)
        // Let's loop from -6 to 6 (13 times), printing out each x value
test:   cmp    x19, 6
        b.ge   done                 // If [loop counter] >= 6, exit loop and branch to "done"
        ...
        mul    x22, x21, x21        // x22 = x * x
        mov    x20, 27              // x20 = 27
        mul    x22, x22, x20        // x22 = x^2 * 27
        add    x23, x23, x22        // x23 = 5x^3 + 27x^2
        // -27x
        mov    x20, -27             // x20 = -27
        mul    x22, x20, x21        // x22 = -27 * x
        add    x23, x23, x22        // x23 = 5x^3 + 27x^2 - 27x 
        // 5x^3 + 27x^2 - 27x - 43
        mov    x20, -43             // x20 = -43
