...
define(totalRatio_r, d22)
define(ratio_r, d23)
define(num_r, d24)
define(denom_r, d25)
define(loop_r, d26)
define(counter_r, d27)
define(input_r, d28)
define(changeSign_r, d29)
define(sign_r, d30)
define(xToe, d16)
define(xTone, d17)
// Assembler equates
...
main:	stp	    x29, x30, [sp, alloc]!  // Save frame pointer (fp, x29) and link register (lr, x30) to stock, allocating 16 bytes, ore-increment SP
	    ...
	    fmov	loop_r, 1.0e+0          // Initialize loop counter to 1
...
top:	mov	    w0, fd_r		        // 1st arg (fd), reads from this file
	    mov	    x1, buf_base_r		    // 2nd arg (buf), stores it here
	    mov	    w2, buf_size		    // 3rd arg (n), reads 8 bytes at a time 
	    mov	    x8, 63			        // read I/O request, 63 is call for read
	    svc	    0			            // call system function
	    mov	    nread_r, x0		        // record $ of bytes actually read, result stored in x0, result in this case returns how many bytes were actually read
	    // Do error checking for read()
	    cmp	    nread_r, buf_size	    // if nread != 8, then check to make sure data matches buffer size, only way to exit is if reached the end of file
	    b.ne	end			            // read failed, branch to exit
//--Start calculation------------------------------------------------------------------------------------------------------------------------------------------
	    //Caluculates e^x
	    ldr	    d0, [buf_base_r]        // load the x	
	    fmov    sign_r, 1.0             // intialize the sign
	    fmov    d1, sign_r              // move the sign into second argument
	    bl	    calc                    // call calc
	    fmov    xToe, d0                // xToe is the result for e^x
	    //Caluculates e^-x
	    ldr	    d0, [buf_base_r]        // load the x
	    fmov    sign_r, -1.0            // intialize the sign
	    fmov    d1, sign_r              // move the sign into second argument
	    bl	    calc                    // call calc
	    fmov    xTone, d0               // xTone is the result for e^x
//--End calculation------------------------------------------------------------------------------------------------------------------------------------------
	    ...
	    ldr	    d0, [buf_base_r]	    // 2nd arg (the long int), load the read 8 bytes from the buffer, d register for double precision 
	    fmov    d1, xToe                // 3rd arg (the e^x)
	    fmov    d2, xTone               // 4th arg (the e^-x)
	    bl	    printf			        // Call the printf() function
	    b	    top			            // branch to top
...
calc:   stp	    x29, x30, [sp, -16]!    // Save frame pointer (fp, x29) and link register (lr, x30) to stock, allocating 16 bytes, ore-increment SP
	    ...
	    fmov    input_r, d0             // x value
	    fmov    sign_r, d1              // sign(+ or -)
	    fmov    changeSign_r, 1.0
	    fmov    num_r, input_r
	    fmov    counter_r, 1.0
	    fmov    denom_r, 1.0
	    fmov    totalRatio_r, xzr
top2:   fdiv    ratio_r, num_r, denom_r
	    //continue if ratio_r bigger than the limit constant 1.0e-10
	    adrp    x26, const_m            // get the base address of our pointer
	    add	    x26, x26, :lo12:const_m
	    ldr	    loop_r, [x26]           // load the value of top into w1 register
	    fcmp    ratio_r, loop_r         // if the ratio_r bigger than the limit constant 1.0e-10, continue
	    b.le    addOne                  // else, branch to addOne
	    //adjust sign of ratio based on input
	    fmul    changeSign_r, changeSign_r, sign_r    // change the sign
	    fmul    ratio_r, ratio_r, changeSign_r
	    fadd    totalRatio_r, totalRatio_r, ratio_r // all ratios calculated individually and then added to totalRatio_r
	    //calculate next numerator and denominator 
	    fmul    num_r, input_r, num_r   // numerator will be multiplied by original input for every iteration
	    fmov    loop_r, 1.0             // intialize loop counter back to 1.0
	    fadd    counter_r, counter_r, loop_r    // counter++
	    fmul    denom_r, denom_r, counter_r //denominator multiplied by the incremented counter for every iteration
	    b	    top2                    // brach back to top
        // Add one to totalRatio_r then exit
addOne: fmov    loop_r, 1.0             // intialize loop counter back to 1.0
	    fadd    totalRatio_r, totalRatio_r, loop_r  // totalRatio++
	    fmov    d0, totalRatio_r        // return totalRatio_r
